...
Opciones principales:
  ...
  --iface IFACE               Interfaz a observar. Si se omite, se infiere por ruta hacia target.
...
[ -n "$IFACE" ] || die "No se pudo inferir interfaz. Use --iface."

# Validacion temprana: evita reportes vacios cuando la interfaz no existe.
# Si el usuario pasa --iface tun0 pero el tunel no esta levantado, es mejor fallar aqui
# y no generar graficos con ceros o capturas fallidas.
if [ ! -d "/sys/class/net/$IFACE" ]; then
  echo "[ERROR] La interfaz '$IFACE' no existe en este host." >&2
  ...
  die "Use una interfaz existente o levante primero el tunel/interfaz correspondiente."
fi

START_STAMP="$(date +"%Y%m%d_%H%M%S")"
RUN_ID="$(safe_name "${LABEL}_${TARGET}_${START_STAMP}")"
...
collect_before_snapshots() {
  run_to_file "${RAW_DIR}/ip_s_link_before.log" ip -s link show dev "$IFACE" || true
...
collect_after_snapshots() {
  run_to_file "${RAW_DIR}/ip_s_link_after.log" ip -s link show dev "$IFACE" || true
...
start_capture() {
  ...
  local pcap="${PCAP_DIR}/${LABEL}_${TARGET}_${PORT}_${IFACE}.pcapng"

  if have tshark; then
    echo "[INFO] Intentando captura con tshark en ${IFACE}..."
    log_cmd "timeout $DURATION tshark -i $IFACE -f '$capture_filter' -w $pcap"
    timeout "$DURATION" tshark -i "$IFACE" -f "$capture_filter" -w "$pcap" > "${RAW_DIR}/tshark_capture.log" 2>&1 &
    local cap_pid=$!
    BG_PIDS="${BG_PIDS:-} $cap_pid"
    echo "$cap_pid:tshark_capture:${RAW_DIR}/tshark_capture.log" >> "${OUT_DIR}/background_pids.txt"
    echo "$pcap" > "${OUT_DIR}/pcap_file.txt"
  elif have tcpdump; then
    echo "[INFO] Intentando captura con tcpdump en ${IFACE}..."
    local pcap_tcpdump="${PCAP_DIR}/${LABEL}_${TARGET}_${PORT}_${IFACE}.pcap"
    log_cmd "timeout $DURATION tcpdump -i $IFACE -w $pcap_tcpdump $capture_filter"
    timeout "$DURATION" tcpdump -i "$IFACE" -w "$pcap_tcpdump" $capture_filter > "${RAW_DIR}/tcpdump_capture.log" 2>&1 &
    local cap_pid=$!
    BG_PIDS="${BG_PIDS:-} $cap_pid"
    echo "$cap_pid:tcpdump_capture:${RAW_DIR}/tcpdump_capture.log" >> "${OUT_DIR}/background_pids.txt"
...
entropy_from_pcap() {
  ...
  local pcap=""
  if [ -f "${OUT_DIR}/pcap_file.txt" ]; then
    pcap="$(cat "${OUT_DIR}/pcap_file.txt")"
  fi
  [ -n "$pcap" ] && [ -f "$pcap" ] || {
...
PY
  ...
  if [ -n "$pcap" ] && [ -f "$pcap" ] && have capinfos; then
    run_to_file "${RAW_DIR}/capinfos.log" capinfos "$pcap" || true
  fi
